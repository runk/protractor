...
TaskRunner.prototype.run = function() {
  ...
  // - Running processes in fork is much more predictable when we want to retry them. Using it by
  //   default even with only 1 spec in queue.
  // - The issue is reporting behavior is different and cannot be so easily controlled
  //   in "forked" mode.
  ...
  var runSpecHelper = function() {
    ...
    runnerFn.call(self, runResults).then(function() {
      ...
      // TaskLogger.flush prints task output to the screen. The reason we don't do it in
      // `_runForked` is we want to avoid non-sense output. If task failed, retried and succedeed
      // we don't want to print any information about errors.
